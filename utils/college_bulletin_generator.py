...
def _draw_page_header(c: canvas.Canvas, width: float, height: float, school_name: str, school_address: str, school_phone: str, school_email: str, school_logo: str, data: dict, compact: bool = False) -> float:
    ...
    if compact:
        ...
        c.drawCentredString(width / 2, title_top_y, f"Bulletin de notes - Collège - suite | {data['student_name']}")
        return subtitle_y - 10

    c.setFillColor(colors.black)
    c.setFont("Helvetica-Bold", 15)
    c.drawCentredString(width / 2, title_top_y, "BULLETIN DE NOTES - COLLEGE")
    c.setFont("Helvetica-Bold", 9)
...
def _draw_subject_group_row(c: canvas.Canvas, table_x: float, table_w: float, y: float, label: str, row_h: float) -> float:
    c.setStrokeColor(PRIMARY_BORDER)
...
def _draw_identity_cards(c: canvas.Canvas, width: float, y: float, data: dict) -> float:
    ...
    photo_path = _resolve_existing_path(data.get("photo_path")) or ""
    inner_margin = 7
