...
class SubjectsPage(QWidget):
    def __init__(self, current_user):
        ...
        self.table.setHorizontalHeaderLabels(["ID", "Matière"])
    ...
    def open_add_dialog(self):
        dialog = AddSubjectDialog(parent=self)
        if dialog.exec():
            self.load_subjects()
    ...
    def open_edit_dialog(self):
        ...
        if selected == -1:
            QMessageBox.warning(self, "Erreur", "Sélectionnez une matière")
            return

        subject_id_item = self.table.item(selected, 0)
        if not subject_id_item:
            QMessageBox.warning(self, "Erreur", "Matière invalide")
            return

        subject_id = subject_id_item.text()

        dialog = EditSubjectDialog(subject_id=subject_id, parent=self)
        if dialog.exec():
            self.load_subjects()