...
class CollegeBulletinsPage(QWidget):
        ...
        try:
            ...
            for row in range(self.table.rowCount()):
                ...
                if not student_id_item:
                    continue
                student_id = int(student_id_item.text())
                generated_files.append(generate_college_bulletin(student_id, term_id))

            if not generated_files:
                QMessageBox.warning(self, "Erreur", "Aucun bulletin n'a été généré.")
                return

            merged_output = f"bulletins/college/{class_label}_{term_label}_classe_complete.pdf"
