...
class AddCycleFeeConfigDialog(QDialog):
    def __init__(self, current_user, parent=None):
        super().__init__(parent)
        self.current_user = current_user
        ensure_cycle_fee_schema()

        self.setWindowTitle("Ajouter un tarif par cycle")
        ...
        try:
            ...
            if establishment_id is None:
                cursor.execute(
                    ...
                    LIMIT 1
                    """,
                    (cycle_id, fee_id, school_year_id),
                )
            else:
                cursor.execute(
                    """
                    ...
                    LIMIT 1
                    """,
                    (establishment_id, cycle_id, fee_id, school_year_id),
                )
            existing = cursor.fetchone()
            if existing:
                cursor.execute(
                    "UPDATE cycle_fee_configs SET amount = %s WHERE id = %s",
                    (amount, int(existing[0])),
                )
            else:
                cursor.execute(
                    """
                    INSERT INTO cycle_fee_configs (establishment_id, cycle_id, fee_id, school_year_id, amount)
                    VALUES (%s, %s, %s, %s, %s)
                    """,
                    (establishment_id, cycle_id, fee_id, school_year_id, amount),
                )
            conn.commit()
            QMessageBox.information(self, "Succès", "Tarif par cycle enregistré.")
            self.accept()
